...
function [Theta_Bragg,Chi_0_Cx,Chi_h_Cx,R_S,E_Scan,G_0H,t_array,k_array,Dwidth]= Bragg_temp_Gui_strain_test_2D(a_Par_strain,b_Par_strain,c_Par_strain,...
    ...
    Range_E_neg,Range_E_pos,Polarization,Ang_asy_Deg_strain,Thickness_strain,N_Step,FBD,crystal_orientation,strain_perp1,strain_perp2,strain_parll)
%%% Calculations
% Constants
...
for i_layers = 1 : layers
    ...
    a_Par = a_Par_strain(1,i_layers) + strain_parll(1,i_layers) * a_Par_strain(1,i_layers);
    b_Par = b_Par_strain(1,i_layers) + strain_perp1(1,i_layers) * b_Par_strain(1,i_layers);
    c_Par = c_Par_strain(1,i_layers) + strain_perp2(1,i_layers) * b_Par_strain(1,i_layers);
...
%         alfa = 2*sin(2*Theta_Bragg) * (Theta-Theta_Bragg) + ((sin(Ang_asy).^2.*tan(Theta_Bragg)-sin(Ang_asy)*cos(Ang_asy)) * strain_perp1(1,i_layers)) + ((sin(Ang_asy).^2.*tan(Theta_Bragg)-sin(Ang_asy)*cos(Ang_asy)) * strain_perp2(1,i_layers)) + ((cos(Ang_asy).^2.*tan(Theta_Bragg) - sin(Ang_asy)*cos(Ang_asy)) * strain_parll(1,i_layers));
%     else
%         alfa = 2*sin(2*Theta_Bragg) * (Theta-Theta_Bragg) + ((sin(Ang_asy).^2.*tan(Theta_Bragg)+sin(Ang_asy)*cos(Ang_asy)) * strain_perp1(1,i_layers)) + ((sin(Ang_asy).^2.*tan(Theta_Bragg)+sin(Ang_asy)*cos(Ang_asy)) * strain_perp2(1,i_layers)) + ((cos(Ang_asy).^2.*tan(Theta_Bragg) + sin(Ang_asy)*cos(Ang_asy)) * strain_parll(1,i_layers));
%     end
...
% Tp(1,:) = MatrixC(1,1,1,:);
% Tp(2,:) = unwrap(imag(expX2));
% Tp(3,:) = unwrap(real(expX2));
% Tp(4,:) = expX2;
% 
% figure
% hold on
% %plot(E_Scan,abs(Tp(1,:)).^2,'b')
% %plot(E_Scan,abs(Tp(2,:)).^2,'r')
% %plot(E_Scan,abs(Tp(3,:)).^2,'b')
% plot(E_Scan,abs(Tp(4,:)).^2,'k')
...
%From the rare surface up
while i_layers > 1
    for i_N_Step = 1:N_Step
        M_calculate01(:,:) = Matrix0(i_layers - 1, :, :, i_N_Step);
        M_calculateC1(:,:) = MatrixC(i_layers - 1, :, :, i_N_Step);
