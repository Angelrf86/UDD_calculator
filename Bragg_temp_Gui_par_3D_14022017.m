function [R_S_3D] = Bragg_temp_Gui_par_3D_14022017(Z,a_Par,b_Par,c_Par,Energy_Bragg,h_Miller,k_Miller,l_Miller,DWF,f_0,f_1,f_2,absor,Range_E_neg,Range_E_pos,Polarization,Thickness,N_Step,FBD,ky_array,kx_array)
...
% kx0 =0.0001*k0_Bragg;
% sigma_kx = 1*k0_Bragg;
% kx0_array = linspace(-NStepx/2,NStepx/2,NStepx)*k0_Bragg;
% kx_array = gaussmf(kx0_array,[sigma_kx kx0_array(1,NStepx/2)])*kx0;
% 
% ky0 =0.0001*k0_Bragg;
% sigma_ky = 1*k0_Bragg;
% ky0_array = linspace(-NStepy/2,NStepy/2,NStepy)*k0_Bragg;
...
%  figure
%  size(R_0H_S_3D);
%  surf(kx_array,ky_array,abs(R_0H_S_3D(:,:,1)).^2)
%  figure
...
% G_0H_S_3D = fftshift(fftshift(fftshift(fftn(R_0H_S_3D),1),2),3);
% G_00_S_3D = fftshift(fftshift(fftshift(fftn(R_00_S_3D),1),2),3);
...
%x_array = dx *linspace(1,N_Step/10,N_Step/10) ;
                
%  dky = ky_array(1,1)-ky_array(1,2);
%  dy = 2*pi/(N_Step/10*dky);
