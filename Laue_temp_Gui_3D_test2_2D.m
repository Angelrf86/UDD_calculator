function [R_S_3D] = Laue_temp_Gui_3D_test2_2D(a_Par,b_Par,c_Par,Energy_Bragg,Energy_center,...
    ...
    for i_k01 = 1:size(k0_vect,2)
        for i_k02 = 1:size(k0_vect,3)
            for i_k03 = 1:size(k0_vect,4)
                %          size(Matrix_Bragg)
                %           size(k0_vect(:,i_k01,i_k02,i_k03))
                for j_k0_beam = 1:3
...
R_S_3D_plot = zeros(size(R_S_3D,1),size(R_S_3D,2));
for i_plot = 1:size(R_S_3D,3)
    if mod(i_plot,100) == 0
        R_S_3D_plot (:,:) = R_S_3D(:,:,i_plot);
        figure
        ky_array2(1,:) = ky_array(1,1,:); 
        kx_array2(1,:) = kx_array(1,:);
        surf(kx_array2,ky_array2,abs(R_S_3D_plot).^2,'edgecolor','none')
        view([90 -90])
    end
end
